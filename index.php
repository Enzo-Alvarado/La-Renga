<?php

//Enviar datos al MAIL de la barberia
if (isset($_POST['enviar'])) {
  if(!empty($_POST['nombre']) && !empty($_POST['email']) && !empty($_POST['comentario'])) {
      $email = "jere@gmail.com";
      $asunto = "Mensaje sobre La Renga";
      $nombre = $_POST['nombre']; 
      $comentario = $_POST['comentario']; 
      $email_usuario = $_POST['email'];
      
      
      $mensaje = "Hola! has recibido un nuevo mensaje sobre La Renga." . "\n";
      $mensaje.= " - Nombre y Apellido: " . $nombre . "\n"; 
      $mensaje.= " - Email: " . $email_usuario . "\n"; 
      $mensaje.= " - Comentario: " . $comentario; 


      $header = "From: " . $email_usuario . "\r\n";
      $header.= "Reply-To: " . $email_usuario . "\r\n";
      $header.= "X-Mailer: PHP/". phpversion();

      mail($email, $asunto, $mensaje, $header);
  }
} 

?>


<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />

    <!--ICONOS-->
    <script src="https://unpkg.com/boxicons@2.1.4/dist/boxicons.js"></script>

    <!--CSS-->
    <link rel="stylesheet" href="style.css" />
    <title>La Renga</title>
  </head>
  <body>
    <header>
      <!--    BARRA DE NAVEGACIÓN     -->
      <nav class="contenedor-navbar">
        <ul class="lista-nav">
          <li class="item-nav"><a href="#">Inicio</a></li>
          <li class="item-nav"><a href="#seccion-biografia">Biografia</a></li>
          <li class="item-nav"><a href="#seccion-musica">Musica</a></li>
          <li class="item-nav"><a href="#seccion-contacto">Contactanos</a></li>
        </ul>
      </nav>
    </header>

    <!--    CUERPO DE LA PÁGINA     -->
    <main>
      <!--    SECCIÓN INICIO      -->
      <section class="seccion-inicio">
        <div class="contenedor-texto-inicio">
          <h1>La Renga</h1>
          <p class="texto-inicio">Somos los mismos de<span> SIEMPRE</span></p>
        </div>
        <!--pequeña descripcion con una frase destacada en el spa con otro color-->
        <div class="contenedor-iconos">
          <!--CONTENEDOR DE LOS ICONOS D3E LAS REDES SOCIALES-->
          <a href="https://www.facebook.com/larenga?mibextid=LQQJ4d"
            ><box-icon
              type="logo"
              name="facebook-circle"
              color="#ffffff"
            ></box-icon
          ></a>
          <a href="https://www.instagram.com/larenga/"
            ><box-icon name="instagram" type="logo" color="#ffffff"></box-icon
          ></a>
          <a href="https://www.youtube.com/@larenga"
            ><box-icon name="youtube" type="logo" color="#ffffff"></box-icon
          ></a>
          <a href="https://open.spotify.com/intl-es/artist/30fEdZPXgWfC4sNttcyB3C?si=hzFLXN_WSyu04TXec05vIw"
            ><box-icon name="spotify" type="logo" color="#ffffff"></box-icon
          ></a>
        </div>
      </section>

      <!--    SECCIÓN BIOGRAFIA     -->
      <section class="seccion-biografia" id="seccion-biografia">
        <div class="titulo-biografia">
          <span>Biografia</span>
          <h3>La Renga</h3>
        </div>
        <div class="texto-biografia">
          <img src="image/imgInicio.jpg" alt="" />
          <div>
          <p>
          La Renga es una banda de rock argentina formada en 1988 en el barrio de Mataderos, Buenos Aires. Nació cuando un grupo de amigos, Gustavo "Chizzo" Nápoli (voz y guitarra), Raúl "Locura" Dilelio (guitarra), Gabriel "Tete" Iglesias (bajo) y Jorge "Tanque" Iglesias (batería), se unieron para hacer música inspirada en el rock pesado y con influencias del blues. En los primeros años, la banda tocaba en pequeños bares y clubes, desarrollando un estilo crudo y directo que rápidamente capturó la atención de un público cada vez más grande.

Su primer álbum, Esquivando charcos (1991), fue grabado de manera independiente y los ayudó a ganar notoriedad en la escena del rock argentino. Con el tiempo, La Renga se consolidó como una de las bandas más importantes del rock nacional, reconocida por sus poderosas presentaciones en vivo, su estilo combativo y sus letras cargadas de crítica social.
          </p>
          </div>
        </div>
        <div class="contenedor-btn">
          <a href="biografia.php">
            <button class="btn-biografia">Ver más</button>
          </a>
        </div>
      </section>

      <!--    SECCIÓN DE LA MÚSICA    -->
      <section class="seccion-musica" id="seccion-musica">
        <div class="contenedor-texto-musica">
          <span>Musica</span>
          <h3>Canciones</h3>
        </div>
        <div class="center-canciones">
          <div class="canciones">
            <div class="currentplaying">
              <svg
                class="spotify"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 64 64"
                width="50px"
                height="50px"
              >
                <radialGradient
                  id="ipdIa4~cOclR8yt_ClW93a"
                  cx="33.34"
                  cy="572.064"
                  r="43.888"
                  gradientTransform="translate(0 -534)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#f4e9c3"></stop>
                  <stop offset=".219" stop-color="#f8eecd"></stop>
                  <stop offset=".644" stop-color="#fdf4dc"></stop>
                  <stop offset="1" stop-color="#fff6e1"></stop>
                </radialGradient>
                <path
                  fill="url(#ipdIa4~cOclR8yt_ClW93a)"
                  d="M51.03,37.34c0.16,0.98,1.08,1.66,2.08,1.66h5.39c2.63,0,4.75,2.28,4.48,4.96	C62.74,46.3,60.64,48,58.29,48H49c-1.22,0-2.18,1.08-1.97,2.34c0.16,0.98,1.08,1.66,2.08,1.66h8.39c1.24,0,2.37,0.5,3.18,1.32	C61.5,54.13,62,55.26,62,56.5c0,2.49-2.01,4.5-4.5,4.5h-49c-1.52,0-2.9-0.62-3.89-1.61C3.62,58.4,3,57.02,3,55.5	C3,52.46,5.46,50,8.5,50H14c1.22,0,2.18-1.08,1.97-2.34C15.81,46.68,14.89,44,13.89,44H5.5c-2.63,0-4.75-2.28-4.48-4.96	C1.26,36.7,3.36,35,5.71,35H8c1.71,0,3.09-1.43,3-3.16C10.91,30.22,9.45,29,7.83,29H4.5c-2.63,0-4.75-2.28-4.48-4.96	C0.26,21.7,2.37,20,4.71,20H20c0.83,0,1.58-0.34,2.12-0.88C22.66,18.58,23,17.83,23,17c0-1.66-1.34-3-3-3h-1.18	c-0.62-0.09-1.43,0-2.32,0h-9c-1.52,0-2.9-0.62-3.89-1.61S2,10.02,2,8.5C2,5.46,4.46,3,7.5,3h49c3.21,0,5.8,2.79,5.47,6.06	C61.68,11.92,60.11,14,57.24,14H52c-2.76,0-5,2.24-5,5c0,1.38,0.56,2.63,1.46,3.54C49.37,23.44,50.62,24,52,24h6.5	c3.21,0,5.8,2.79,5.47,6.06C63.68,32.92,61.11,35,58.24,35H53C51.78,35,50.82,36.08,51.03,37.34z"
                ></path>
                <linearGradient
                  id="ipdIa4~cOclR8yt_ClW93b"
                  x1="32"
                  x2="32"
                  y1="530.096"
                  y2="590.253"
                  gradientTransform="translate(0 -534)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#42d778"></stop>
                  <stop offset=".428" stop-color="#3dca76"></stop>
                  <stop offset="1" stop-color="#34b171"></stop>
                </linearGradient>
                <path
                  fill="url(#ipdIa4~cOclR8yt_ClW93b)"
                  d="M57,32c0,12.837-9.663,23.404-22.115,24.837C33.942,56.942,32.971,57,32,57	c-1.644,0-3.25-0.163-4.808-0.471C15.683,54.298,7,44.163,7,32C7,18.192,18.192,7,32,7S57,18.192,57,32z"
                ></path>
                <path
                  fill="#fff"
                  d="M41.683,44.394c-0.365,0-0.731-0.181-1.096-0.365c-3.471-2.009-7.674-3.105-12.24-3.105	c-2.559,0-5.116,0.364-7.491,0.912c-0.365,0-0.914,0.183-1.096,0.183c-0.914,0-1.461-0.732-1.461-1.462	c0-0.913,0.547-1.463,1.279-1.643c2.923-0.732,5.846-1.096,8.951-1.096c5.116,0,9.866,1.276,13.885,3.655	c0.548,0.364,0.914,0.73,0.914,1.642C43.145,43.847,42.414,44.394,41.683,44.394z M44.241,38.181c-0.547,0-0.912-0.18-1.279-0.364	c-3.835-2.375-9.135-3.839-15.163-3.839c-2.924,0-5.664,0.366-7.674,0.916c-0.549,0.18-0.731,0.18-1.096,0.18	c-1.096,0-1.827-0.912-1.827-1.826c0-1.096,0.549-1.645,1.461-2.009c2.74-0.73,5.481-1.279,9.317-1.279	c6.213,0,12.241,1.463,16.991,4.384c0.73,0.364,1.096,1.096,1.096,1.826C46.069,37.269,45.337,38.181,44.241,38.181z M47.165,30.876	c-0.548,0-0.731-0.182-1.279-0.364c-4.385-2.559-10.961-4.021-17.356-4.021c-3.289,0-6.577,0.366-9.5,1.096	c-0.366,0-0.731,0.182-1.279,0.182c-1.279,0.183-2.193-0.912-2.193-2.192c0-1.279,0.731-2.009,1.644-2.192	c3.471-1.096,7.125-1.462,11.327-1.462c6.943,0,14.25,1.462,19.731,4.567c0.73,0.366,1.278,1.096,1.278,2.193	C49.357,29.961,48.442,30.876,47.165,30.876z"
                ></path>
              </svg>
              <p class="heading">Top 5 Canciones de La Renga</p>
            </div>
            <a href="https://open.spotify.com/intl-es/track/4ObkiTmpsSU31gqGhTb8SH?si=882b87c0219146ec">
              <div class="contenedor-cancion">
                <div class="tema">
                  <p class="nombre-tema">Balada Del Diablo y La Muerte de La Renga</p>
                  <p class="artista">La Renga</p>
                </div>
                <div class="play"></div>
              </div>
            </a>
            <a href="https://open.spotify.com/intl-es/track/24o76WATbnSIhV26hHgUHn?si=ef22e2c4fff14c6e">
              <div class="contenedor-cancion">
                <div class="tema">
                  <p class="nombre-tema">El Revelde</p>
                  <p class="artista">La Renga</p>
                </div>
                <div class="play"></div>
              </div>
            </a>
            <a href="https://open.spotify.com/intl-es/track/3qSsdVel7dNcpW4khZejHa?si=acb0ebd9707041b0">
              <div class="contenedor-cancion">
                <div class="tema">
                  <p class="nombre-tema">Veneno</p>
                  <p class="artista">La Renga</p>
                </div>
                <div class="play"></div>
              </div>
            </a>
            <a href="https://open.spotify.com/intl-es/track/0ljyHIWERJQp8O5LKgKm1l?si=7479cc2aee7b4a4b">
              <div class="contenedor-cancion">
                <div class="tema">
                  <p class="nombre-tema">La Razón Que Te Demora</p>
                  <p class="artista">La Renga</p>
                </div>
                <div class="play"></div>
              </div>
            </a>
            <a href="https://open.spotify.com/intl-es/track/5zdazgWuhzFMMtwt5kdeFD?si=a06dcdfaf92442f4">
              <div class="contenedor-cancion">
                <div class="tema">
                  <p class="nombre-tema">El Final Es En Donde Partí</p>
                  <p class="artista">La Renga</p>
                </div>
                <div class="play"></div>
              </div>
            </a>
          </div>
        </div>

        <div class="contenedor-btn">
          <a href="album.php">
            <button class="btn-biografia">Albums/Discos</button>
          </a>
        </div>
      </section>

      <!--  SECCION CONTACTO    -->
      <section class="seccion-contacto" id="seccion-contacto">
        <div class="contenedor-texto-contacto">
          <span>Contacto</span>
          <h3>Comunicate</h3>
        </div>
        <div class="contenedor-formulario">
          <form action="" method="post">
            <div class="input-1">
              <input
                type="text"
                name="nombre"
                id="nombre"
                placeholder="Nombre y Apellido"
              />
              <input type="email" name="email" id="email" placeholder="Email" />
            </div>
            <div class="input-2">
              <textarea
                name="comentario"
                id="comentario"
                placeholder="Escribe un comentario para La Renga"
              ></textarea>
              <input
                class="btn-form"
                type="submit"
                name="enviar"
                id="enviar"
                value="ENVIAR"
              />
            </div>
          </form>
        </div>
      </section>
    </main>

    <!--    FOOTER O PIE DE PAGINA     -->

    <footer>
      <p>Desarrollado por Enzo Alvarado</p>
      <p>©Derechos reservados</p>
    </footer>
  </body>
</html>
